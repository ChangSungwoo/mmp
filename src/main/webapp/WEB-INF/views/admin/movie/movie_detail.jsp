...
<body>
	<div class="wrapper">
		...
		<jsp:include page="/WEB-INF/views/admin/include/includeNavi.jsp" flush="true">
			<jsp:param name="thisName" value='<%=java.net.URLEncoder.encode("영화 상세 정보")%>'/>
		...
		<div class="container content height-500">
				<div class="col-md-12">
					<!-- General Unify Forms -->
					<form id="movieEditForm" name="movieEditForm" class="sky-form">
							...
							<div class="row">
								<section class="col col-6">
									...
									<label class="input state-success">
										<input type="text" name="movieTitle" id="movieTitle" value="${movie.movieKorTitle}">
									</label>
								</section>
								<section class="col col-6">
									<label class="label">영화 제목(영문)</label>
									<label class="input">
										<input type="text" name="movieEngTitle" id="movieEngTitle" value="${movie.movieEngTitle}">
									</label>
								</section>
							</div>
							<div class="row">
								<section class="col col-6">
									...
									<label class="input state">
										<input type="text" name="directorText" id="directorText" value="${movie.directorText}">
									</label>
								</section>
								<section class="col col-6">
									<label class="label">배우 정보</label>
									<label class="input">
										<input type="text" name="actorText" id="actorText" value="${movie.actorText}">
									</label>
								</section>
							</div>
							<div class="row">
								<section class="col col-6">
									...
									<label class="input state-success"">
										...
										<input type="text" name="releaseDt" id="releaseDt"  value="${movie.releaseDt}">
									</label>
								</section>
								<section class="col col-6">
									<label class="label">영화등록일자</label>
									<label class="input state-success"">
										<i class="icon-append fa fa-calendar"></i>
										<input type="text" name="creDt" id="creDt" value="${fn:substring(movie.creDt, 0,10)}">
									</label>
								</section>							
							</div>
							<div class="row">
									...
									<c:choose>
										<c:when test="${fn:length(list) > 0}">
											<c:forEach items="${list}" var="row">
												<label class="radio"><input type="radio" name="stdPriceDiv" id="stdPriceDiv" value="${row.dSeq}" <c:if test="${movie.stdPriceDiv eq row.dSeq }"> checked </c:if>><i class="rounded-x"></i>${row.DName}원</label>
											</c:forEach>
										</c:when>
									</c:choose>
									</div>
	...
	<!-- JS Customization -->
	<script type="text/javascript" src="/js/admin/movie/movie_detail.js"></script>
	<!-- JS Page Level -->
