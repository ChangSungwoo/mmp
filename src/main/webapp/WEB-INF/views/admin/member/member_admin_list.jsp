...
<body>
	...
	<div class="wrapper">
		...
		<jsp:include page="/WEB-INF/views/admin/include/includeNavi.jsp" flush="true">
			<jsp:param name="thisName" value='<%=java.net.URLEncoder.encode("관리자 목록조회")%>'/>
				...
				<div class="col-md-12">
					<div class="panel panel-grey margin-bottom-40">
						<div class="panel-heading">
							<h3 class="panel-title"><i class="fa fa-gear"></i>관리자정보조회</h3>
						</div>
								...
								<c:choose>
									<c:when test="${fn:length(list) > 0}">
										<c:forEach items="${list}" var="row">
											<tr>
												<td>${row.loginId}</td>
												<td><a href="javascript:movieDetail('${row.userNo}');">${row.userNm}(${row.userNickNm})</a></td>
												<td>${row.userNickNm}</td>
												<td>${row.providerCd}</td>
												<td>${row.userGradeCd}</td>
												<td>${row.creDt}</td>
											</tr>
										</c:forEach>
									</c:when>
	...
	<!-- JS Customization -->
	<script type="text/javascript" src="/js/admin/member/member_admin_list.js"></script>
	<!-- JS Page Level -->
